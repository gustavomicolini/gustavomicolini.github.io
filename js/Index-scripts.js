...
function pidoDato() {
	let textoenvio = {
		usuario:'usuario',
		mensaje:'Hola ServerIO, mandame dato'
		};
	
	//emit = cliente envia informacion solo al servidor, otro cliente no lo recibe
	socket.emit('enviarMensaje',textoenvio , function(respuesta) { //respuesta del callback
		//Imprimo texto enviado
		console.log('usuario, ',textoenvio.mensaje);
		document.getElementById("demo4").innerHTML = textoenvio.mensaje;
		//Imprimo respuesta
